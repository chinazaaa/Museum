...
import EditItem from './components/EditItem2';
...
const App: React.FC = () => {
  ...
  const handleCreateItem = async (itemToCreate: ItemType) => {
    ...
    const response = await fetch('/museum-inventory',
    {
      method: 'POST',
  ...
  const handleDeleteItem = async (id: number | string) => {
    ...
    try{
      const response = await fetch(`/museum-inventory/${id}`, //note this URL
      {
        method: 'DELETE',
        //mode: 'cors', // no-cors, *cors, same-origin
        //cache: 'no-cache', // *default, no-cache, reload, force-cache, only-if-cached
  ...
  const handleUpdateItem = async(editedItem: ItemType) => {
    ...
    //let's try to write to backend
    try {
      const response = await fetch(`/museum-inventory/${editedItem.id}`,
      {
